...
public class AutoStartPos2 extends SequentialCommandGroup {
   ...
   /** 
   * Creates a new SequentialCommandGroup AutoStartPos2
   * positive angles turn right, negative angles turn left
   *
   * @param driveTrain The DriveTrain shooter intake delivery subsystems this command will run on
   */
  public AutoStartPos2(DriveTrain driveTrain, Shooter shooter, Intake intake, Delivery delivery) {

    addCommands(
      new InstantCommand(() -> DriveTrain.stop(), driveTrain),    // make sure stopped
      new InstantCommand(() -> Shooter.setSpeed(-0.6), shooter),   //Turn shooter on
      new WaitCommand(1.5),
      ...
      new InstantCommand(() -> Shooter.stop(), shooter),       //Turn shooter off
      new AutoDriveStraightTime(-0.6, 2), //Get out of the way
      new InstantCommand(() -> DriveTrain.stop(), driveTrain)  // make sure stopped
    );
  }
}

...
public class AutoTurnToAngle extends CommandBase {
   ...
   */
  public AutoTurnToAngle(double turnPowerIn, double targetAngleIn) {
  ...
  public void initialize() {
    ...
    DriveTrain.doArcadeDrive(0.0, 0.0);  // make sure robot is stopped
    DriveTrain.resetGyro();  // reset gyro so 0 is current heading
    inRange = false;
    System.out.println("**starting AutoTurnToAngle target angle: " +targetAngle +" heading: "+String.format("%.3f", m_driveTrain.getHeadingAngle()));
  }
  ...
  public void execute() {
    ...
    if (inRange) {
      DriveTrain.doArcadeDrive(0.0, 0.0);  // in range so stop 
      ...
      if (currentDiff <= 0) {
        DriveTrain.doArcadeDrive(0.0, -turnRate);  // turn to left ( negative rotation) 
        if (counter2++ % 3 == 0) { System.out.println("**turn Left Correction - heading: "+String.format("%.3f", currentHeading)); }

      } else {
        DriveTrain.doArcadeDrive(0.0, turnRate);  // turn to right ( positive rotation)
  ...
  public void end(boolean interrupted) {
    ...
    System.out.println("**ending AutoTurnToAngle command  current heading: " + String.format("%.3f", currentHeading));
  }
